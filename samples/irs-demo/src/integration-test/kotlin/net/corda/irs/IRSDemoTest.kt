    ...
    fun `runs IRS demo`() {
        driver(useTestClock = true, isDebug = true) {
            val notaryFuture = startNode(
                    DUMMY_NOTARY.name,
                    setOf(ServiceInfo(SimpleNotaryService.type), ServiceInfo(NodeInterestRates.Oracle.type))).toCompletableFuture()
            val nodeAFuture = startNode(DUMMY_BANK_A.name, rpcUsers = listOf(rpcUser)).toCompletableFuture()
