...
class BankOfCordaRPCClientTest {
    ...
    fun `issuer flow via RPC`() {
        driver(dsl = {
            ...
            val nodeBankFuture = startNode(BOC.name, setOf(ServiceInfo(SimpleNotaryService.type)), listOf(bocManager)).toCompletableFuture()
            val nodeBigCorpFuture = startNode(BIGCORP_LEGAL_NAME, rpcUsers = listOf(bigCorpCFO)).toCompletableFuture()
