...
class BankOfCordaHttpAPITest {
    ...
    fun `issuer flow via Http`() {
        driver(dsl = {
            val nodeBOCFuture = startNode(BOC.name, setOf(ServiceInfo(SimpleNotaryService.type))).toCompletableFuture()
            val nodeBigCorpFuture = startNode(BIGCORP_LEGAL_NAME).toCompletableFuture()
