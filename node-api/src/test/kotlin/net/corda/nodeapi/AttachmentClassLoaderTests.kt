...
class AttachmentClassLoaderTests : TestDependencyInjectionBase() {
    companion object {
        ...
        val ISOLATED_CONTRACT_CLASS_NAME = "net.corda.finance.contracts.isolated.AnotherDummyContract"
    ...
    fun `testing Kryo with ClassLoader (without top level class name)`() {
        ...
        val context = P2P_CONTEXT.withClassLoader(cl).withWhitelisted(Class.forName(ISOLATED_CONTRACT_CLASS_NAME, true, cl))
        ...
        val context3 = P2P_CONTEXT.withClassLoader(cl2).withWhitelisted(Class.forName(ISOLATED_CONTRACT_CLASS_NAME, true, cl2))
    ...
    fun `test serialization of WireTransaction with dynamically loaded contract`() {
        ...
        val contract2 = contractClassloader.loadClass(copiedWireTransaction.getOutput(0).contract).newInstance() as DummyContractBackdoor
