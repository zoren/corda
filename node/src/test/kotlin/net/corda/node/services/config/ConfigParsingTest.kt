...
class ConfigParsingTest {
    ...
    @Test
    fun `Enum`() {
        val config = config("value" to TestEnum.Value1.name)
        val data = EnumData(TestEnum.Value1)
        assertThat(config.parseAs<EnumData>()).isEqualTo(data)
        assertThat(data.toConfig()).isEqualTo(config)
    }
    ...
    @Test
    fun `List of Enum`() {
        testValueList<EnumListData>(TestEnum.Value2, TestEnum.Value1, valuesToString = true)
    }
    ...
    private inline fun <reified T : ListData<Any>> testValueList(value1: Any, value2: Any, valuesToString: Boolean = false) {
        ...
        for (n in 0..2) {
            ...
            assertThat(config.parseAs<T>()).`as`(data.toString()).isEqualTo(data)
            assertThat(data.toConfig()).`as`(data.toString()).isEqualTo(config)
        }
    }
    ...
    data class EnumData(val value: TestEnum)
