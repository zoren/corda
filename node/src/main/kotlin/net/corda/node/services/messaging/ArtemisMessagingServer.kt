...
class ArtemisMessagingServer(override val config: NodeConfiguration,
    ...
    private fun deployBridge(address: ArtemisPeerAddress, legalNames: Set<X500Name>) {
     ...
     * P2P address.
     */
    private fun deployBridge(queueName: String, target: NetworkHostAndPort, legalNames: Set<X500Name>) {
    ...
    internal fun hostVerificationFail(expectedLegalNames: Set<X500Name>, errorMsg: String?) {
        log.error(errorMsg)
...
private class VerifyingNettyConnector(configuration: MutableMap<String, Any>,
    ...
    override fun createConnection(): Connection? {
        ...
        if (sslEnabled && connection != null) {
            ...
            val expectedLegalNames = (configuration[ArtemisTcpTransport.VERIFY_PEER_LEGAL_NAME] ?: emptySet<X500Name>()) as Set<X500Name>
            try {
