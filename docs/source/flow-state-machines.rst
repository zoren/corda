...
a ``receive`` or ``sendAndReceive``. This received exception can be caught by the receiver or allowed to propagate in its
call stack, thus terminating it. If the receiver was invoked via ``subFlow`` (details below) then the exception can
also be caught there, enabling re-invocation of the sub-flow.
...
done in the ``unwrap`` method of the received ``UntrustworthyData``. In the above example the seller verifies the price
by calling ``checkData`` which throws ``IllegalDataException``, a subtype of ``FlowException``, if the price is invalid.
It's then up to the buyer to either try again with a better price or give up.
